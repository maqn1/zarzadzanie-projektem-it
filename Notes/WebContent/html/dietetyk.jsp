			...
			<div class="container">
				...
				<table align="center" class="tg">
					...
					<tr>
						...
						<th class="tg-jb24">Edytuj dane</th>
						<th class="tg-jb24">Oblicz</th>
				 	</tr>
					<%
						try{
							connection = DriverManager.getConnection(url, user, pass);
							statement = connection.createStatement();
							String sql = "SELECT ID_DANYCH, IMIE, NAZWISKO, MASA_CIALA, WZROST, WIEK, AKTYWNOSC, PLEC FROM DANE;";
						
							resultSet = statement.executeQuery(sql);
							while(resultSet.next()){
					%>
					<tr>
						<td class="tg-i81m"><%=resultSet.getString("ID_DANYCH") %></td>
						<td class="tg-baqh"><%=resultSet.getString("IMIE") %></td>
						<td class="tg-baqh"><%=resultSet.getString("NAZWISKO") %></td>
						<td class="tg-baqh"><%=resultSet.getString("MASA_CIALA") %></td>
						<td class="tg-baqh"><%=resultSet.getString("WZROST") %></td>
						<td class="tg-baqh"><%=resultSet.getString("WIEK") %></td>
						<td class="tg-baqh"><%=resultSet.getString("AKTYWNOSC") %></td>
						<td class="tg-baqh"><%=resultSet.getString("PLEC") %></td>
						<td class="tg-r31x">
							<form action="DaneEdycja.jsp">
								<input type="hidden" name="idDanych" value="<%=resultSet.getString("ID_DANYCH") %>"/>
								<input type="submit" value="EDYTUJ" class="btn-edit"/>
							</form>
						</td>
						<td class="tg-r31x">
							<form action="oblicz.jsp">
								<input type="hidden" name="idDanych" value="<%=resultSet.getString("ID_DANYCH") %>"/>
								<input type="submit" value="OBLICZ" class="btn-remove"/>
							</form>
						</td>
				  	</tr>
				  	<%	
